...
"      Version: 2.1.0
"      Updater: marslo
"   LastChange: 2026-05-14 05:03:50
"      History:
...
function! s:AddTitle(exist_created)
    ...
    call add(l:lines, l:pre . '   LastChange : ' . strftime('%Y-%m-%d %H:%M:%S'))
    call add(l:lines, l:pre . '=============================================================================' )
...
function! s:UpdateAuthorInfo()
    ...
    " --- 步骤 A: 纯内存扫描 (不要在这里调用任何 system() 或 s:GetBirthTime) ---
    ...
    if !l:has_header
        ...
        while l:n <= l:max
            ...
            elseif l:line =~# 'LastChange\s*:'
                let l:newline = substitute(l:line, ':\(\s*\)\(\S.*$\)$', ':\1' . strftime('%Y-%m-%d %H:%M:%S'), 'g')
            endif
    ...
    " --- 步骤 C: 清理并强制同步状态 ---
    " 在恢复环境前，告诉 Neovim 缓冲区是权威的
    if has('nvim')
        silent! noautocmd write! " 这是一个狠招：如果还在报错，尝试在 diffupdate 前静默写入一次
    endif
